/* ==============================================================================
 Création des vues cumulative des données SIRIUS de l'hopital Européen Goerge-Pompidou
   2015-2016 et 2019
 SGBD          : PostgreSQL 9.6+
...
/* ==============================================================================
 ...
 ================================================================================ */
CREATE OR REPLACE VIEW "hegp_pole" AS
WITH pole_2019 AS
(
  SELECT DISTINCT
...
CREATE OR REPLACE VIEW "hegp_serviceDepartement" AS
...
CREATE OR REPLACE VIEW "hegp_SpecialiteMedical" AS
WITH spe_2019 AS
(
  SELECT DISTINCT
...
CREATE OR REPLACE VIEW "hegp_uniteFonctionnelle" AS
WITH uf_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
CREATE OR REPLACE VIEW "hegp_uniteFonctionnelle-service" AS
WITH uf_sd_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
/* ==============================================================================
 ...
 ================================================================================ */
CREATE OR REPLACE VIEW "hegp_uniteGestion" AS
WITH ug_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
CREATE OR REPLACE VIEW "hegp_uniteFonctionnelle-uniteGestion" AS
WITH uf_ug_2019 AS
(
  SELECT DISTINCT
...
CREATE OR REPLACE VIEW "hegp_uniteMedicoAdministrative" AS
WITH uma_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
/* ==============================================================================
 ...
 ================================================================================ */
 -- NOTE 2019-04-19 CK : ...
CREATE OR REPLACE VIEW "hegp_uniteAdministrative" AS
WITH ua_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
(
  SELECT DISTINCT
    ...
    ,'2017' AS fileDate
  FROM "hupo_2017"
  WHERE "CODE HOPITAL" = '075' AND "CODE UA" <> '' AND "CODE UA" <> '-'
), ua_2016 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
CREATE OR REPLACE VIEW "hegp_uniteAdministrative-unitMedicoAdministrative" AS
WITH ua_uma_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
(
  SELECT DISTINCT
    ...
    "CODE POLE" AS pole_code,
    "CODE UA" AS ua_code,
    "LIBELLE UA" AS ua_label,
    "DATE EFFET CREA UA",
    "DATE EFFET MOD UA",
    "DATE EFFET FIN VAL UA",
    createValidTimePeriod("DATE EFFET CREA UA", "DATE EFFET FIN VAL UA", "DATE EFFET MOD UA") AS ua_vt,
    "CODE UMA" AS uma_code,
    "LIBELLE UMA" AS uma_label,
    "TYPE UMA",
    "TYPE ACTIVITE",
    "Champ PMSI",
    "URM CLINIQUE",
...
(
  SELECT DISTINCT
    ...
    "DATE EFFET MOD UMA",
    "DATE EFFET FIN VAL UMA",
    createValidTimePeriod("DATE EFFET CREA UA", "DATE EFFET FIN VAL UA", "DATE EFFET MOD UA") AS uma_vt
    ,'2016' AS fileDate
  FROM "ges-hegp_2016"
  WHERE "CODE UMA" <> '' AND "CODE UMA" <> ''
), ua_uma_2015 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
UNION
SELECT * FROM ua_uma_2017
UNION
SELECT * FROM ua_uma_2016
UNION
SELECT * FROM ua_uma_2015
;
-- Les liens entre des unités administrative et des unités médico-administrative
CREATE OR REPLACE VIEW "hegp_uniteAdministrative-service" AS
WITH ua_sd_2019 AS
(
  SELECT DISTINCT
    ges."CODE HOPITAL" AS hopital_code,
...
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
    "CODE POLE" AS pole_code,
    "CODE SERV" AS sd_code,
    "LIBELLE SERV" AS sd_label,
    createValidTimePeriod("DATE EFFET CREA UH", "DATE EFFET FIN VAL UH", "DATE EFFET MOD UH") AS sd_vt,
    "CODE UA" AS ua_code,
    "LIBELLE UA" AS ua_label,
    createValidTimePeriod("DATE EFFET CREA UA", "DATE EFFET FIN VAL UA", "DATE EFFET MOD UA") AS ua_vt
    ,'2017' AS fileDate
  FROM "hupo_2017"
  WHERE "CODE UA" <> '' AND "CODE UA" <> '-' AND "CODE SERV" <> '' AND "CODE SERV" <> '-'
),
ua_sd_2016 AS
(
  SELECT DISTINCT
...
UNION
SELECT * FROM ua_sd_2017
UNION
SELECT * FROM ua_sd_2016
UNION
SELECT * FROM ua_sd_2015
;
/* ==============================================================================
 Les données des unités d'hospitalisation
 ================================================================================ */
-- NOTE 2019-04-03 CK : Les UMA non pas obligatoirement un UH.
CREATE OR REPLACE VIEW "hegp_uniteHospitaliere" AS
WITH uh_2019 AS
(
  SELECT
    "CODE HOPITAL" AS hopital_code,
...
CREATE OR REPLACE VIEW "hegp_uniteHospitaliere-service" AS
WITH uh_sd_2019 AS
(
  SELECT DISTINCT
    ges."CODE HOPITAL" AS hopital_code,
...
(
  SELECT DISTINCT
    ...
    createValidTimePeriod("DATE EFFET CREA UH", "DATE EFFET FIN VAL UH", "DATE EFFET MOD UH") AS sd_vt,
    "CODE UH" AS uh_code,
    "LIBELLE UH" AS uh_label,
    createValidTimePeriod("DATE EFFET CREA UH", "DATE EFFET FIN VAL UH", "DATE EFFET MOD UH") AS uh_vt
    ,'2017' AS fileDate
  FROM "hupo_2017"
  WHERE "CODE UH" <> '' AND "CODE UH" <> '-' AND "CODE SERV" <> '' AND "CODE SERV" <> '-'
),
uh_sd_2016 AS
(
  SELECT DISTINCT
...
UNION
SELECT * FROM uh_sd_2017
UNION
SELECT * FROM uh_sd_2016
UNION
SELECT * FROM uh_sd_2015
;

-- Les liens entre des unité hospitalières et des unités administrative
CREATE OR REPLACE VIEW "hegp_uniteHospitaliere-uniteAdministrative" AS
WITH uh_ua_2019 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
(
  SELECT DISTINCT
    ...
    ,'2017' AS fileDate
  FROM "hupo_2017"
  WHERE "CODE HOPITAL" = '075' AND "CODE UH" <> '' AND "CODE UH" <> '-'
    AND "CODE UA" <> '' AND "CODE UA" <> '-'
), uh_ua_2016 AS
(
  SELECT DISTINCT
    "CODE HOPITAL" AS hopital_code,
...
UNION
SELECT * FROM uh_ua_2017
UNION
SELECT * FROM uh_ua_2016
UNION
SELECT * FROM uh_ua_2015
;
/* ==============================================================================
 Les données des unités de responsabilité clinique (URM)
 ================================================================================ */
CREATE OR REPLACE VIEW "hegp_uniteResponsabiliteClinique" AS
WITH urm_2019 AS
(
  SELECT
    "CODE HOPITAL" AS hopital_code,
...
(
  SELECT
    ...
    ,'2017'::CHAR(4) AS fileDate
  FROM "hupo_2017"
  WHERE "CODE HOPITAL" = '075' AND "CODE UMA" <> '' AND "URM CLINIQUE" <> ''
), urm_2016 AS
(
  SELECT
    "CODE HOPITAL" AS hopital_code,
...
UNION
SELECT * FROM urm_2017
UNION
SELECT * FROM urm_2016
UNION
SELECT * FROM urm_2015
;
