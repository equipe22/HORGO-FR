...
WITH etablissement AS
    ...
    "hopital" as label,
    daterange('1923-11-01', null) AS vt
    FROM "sirius"."ges-hap_2019"
    )
...
FROM service
;
-- Création des liens entre service et pole
...
(
  ...
  FROM "sirius"."hap_SpecialiteMedical"
  WHERE filedate = '2019'
)
...
FROM uf
;
-- Création du lien unité fonctionnelle - specialité médicale
...
FROM ufspe
;
-- Création du lien unité fonctionnelle - service
WITH ufservice AS
(
  SELECT DISTINCT
    hopital_code,
  ...
  FROM "sirius"."hap_uniteFonctionnelle-service"
...
;
--
-- Création des unités fonctionnelles - unité de gestion
-- NOTE 2019-04-04 CK : La période de validité du lien entre une UF et UG est la période de validité la plus petite
--   des deux unités.
...
(
  ...
  FROM "sirius"."hap_uniteFonctionnelle-uniteGestion"
  WHERE filedate = '2019'
)
...
-- NOTE 2019-04-08 CK : un UMA peut être une unité de deux UG.
WITH uma AS
(
  SELECT DISTINCT
    hopital_code,
  ...
  WHERE filedate = '2019' --AND uma_code <> '000'
)
...
FROM uaDiscipline
;
-- Création des lien unités administratives - unités médico-Administrative
--
WITH ua_uma AS
(
  SELECT DISTINCT
  ...
  FROM "sirius"."hap_uniteAdministrative-unitMedicoAdministrative"
  WHERE filedate = '2019'
)
...
(
  ...
  FROM "sirius"."hap_uniteAdministrative-service"
  WHERE filedate = '2019'
)
...
(
  ...
  FROM "sirius"."hap_uniteHospitaliere-service"
  WHERE filedate = '2019'
)
...
(
  ...
  FROM "sirius"."hap_uniteHospitaliere-uniteAdministrative"
  WHERE filedate = '2019'
)
